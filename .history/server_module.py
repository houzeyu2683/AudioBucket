...
class Audio:
    ...
    def collect(self, path: str) -> tuple:
        table = pandas.read_csv(path) # .cache/houzeyu2683/1d3f2sxf.csv
        if(True):
...
with gradio.Blocks(title=title, theme=theme) as service:
    Home(user=secret['user']).renderComponent()
    pass
topic = 'Audio'
with service.route(f"{topic}", f"/{topic}"):
    Audio(user=secret['user']).renderComponent()
    gradio.Request
    pass
